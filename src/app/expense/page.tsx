...
import { Plus, ArrowDownRight, Search, Download, Edit, Trash2 } from "lucide-react";
...
const emptyForm: Transaction = { id: "", date: "", type: "expense", amount: 0, category: "Operasional Gudang", notes: "" };
...
export default function ExpensePage() {
  ...
  useEffect(() => {
    ...
    if (stored) {
      ...
      setExpenseList(parsed.filter((t: Transaction) => t.type === "expense"));
    }
  }, []);

  const saveTransactions = (newData: Transaction[]) => {
    setAllTransactions(newData);
    setExpenseList(newData.filter((t) => t.type === "expense"));
  ...
  return (
    <div className="max-w-6xl mx-auto space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700 pb-12">
      {deleteModalId && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm animate-in fade-in zoom-in-95 duration-200">
          <div className="bg-white p-6 rounded-2xl w-full max-w-sm shadow-xl">
            <h3 className="text-lg font-semibold text-gray-900 mb-2">Hapus Pengeluaran</h3>
            <p className="text-gray-500 text-sm mb-6">Apakah Anda yakin ingin menghapus data pengeluaran ini? Data yang terhapus tidak dapat dikembalikan.</p>
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          <h1 className="text-2xl font-semibold text-gray-900">Pengeluaran (Expense)</h1>
          <p className="text-gray-500 mt-1">Kelola data tagihan dan aliran kas keluar SECERA.</p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 w-full max-w-2xl">
        <div className="bg-red-50/50 border border-red-100 p-6 rounded-2xl shadow-sm">
          <p className="text-sm text-red-800 font-medium mb-1">Total Pengeluaran (Tercatat)</p>
          <h3 className="text-3xl font-bold text-red-600 tracking-tight">Rp {totalExpense.toLocaleString("id-ID")}</h3>
        </div>
        <div className="bg-gray-50 border border-gray-100 p-6 rounded-2xl shadow-sm">
          ...
          <h3 className="text-3xl font-bold text-gray-900">{expenseList.length} Transaksi</h3>
        </div>
      </div>
      ...
      {isModalOpen && (
        <div className="bg-white p-6 rounded-2xl border border-gray-100 shadow-sm mb-8 animate-in slide-in-from-top-4">
          <div className="flex justify-between items-center mb-6">
            <h2 className="text-lg font-semibold text-gray-900">{formData.id ? "Edit Pengeluaran" : "Form Pengeluaran Baru"}</h2>
          ...
          <form className="grid grid-cols-1 md:grid-cols-4 gap-6" onSubmit={handleSubmit}>
            <div className="space-y-2 md:col-span-2">
              <label className="text-sm font-medium text-gray-700">Nominal Pengeluaran (Rp)</label>
              <input type="number" value={formData.amount || ""} onChange={(e) => setFormData({ ...formData, amount: Number(e.target.value) })} placeholder="Contoh: 500000" className="w-full bg-gray-50 border border-gray-200 rounded-lg px-4 py-2.5 text-sm focus:ring-2 focus:ring-gray-900 focus:border-transparent outline-none" required />
            </div>
            <div className="space-y-2 md:col-span-2">
            ...
            <div className="space-y-2 md:col-span-2">
              ...
              <select value={formData.category} onChange={(e) => setFormData({ ...formData, category: e.target.value })} className="w-full bg-gray-50 border border-gray-200 rounded-lg px-4 py-2.5 text-sm focus:ring-2 focus:ring-gray-900 focus:border-transparent outline-none">
                <option>Operasional Gudang/Toko</option>
                <option>Restock Suplier / Kulakan</option>
                <option>Gaji Karyawan</option>
                <option>Biaya Marketing / Iklan</option>
                <option>Pengeluaran Lain-lain</option>
              </select>
            </div>
            <div className="space-y-2 md:col-span-2">
              <label className="text-sm font-medium text-gray-700">Catatan/Deskripsi</label>
              <input type="text" value={formData.notes} onChange={(e) => setFormData({ ...formData, notes: e.target.value })} placeholder="Catatan tagihan..." className="w-full bg-gray-50 border border-gray-200 rounded-lg px-4 py-2.5 text-sm focus:ring-2 focus:ring-gray-900 focus:border-transparent outline-none" />
            </div>
            <div className="md:col-span-4 flex justify-end mt-4 gap-3">
              ...
              <button type="submit" className="bg-gray-900 text-white px-8 py-2.5 rounded-lg text-sm font-medium hover:bg-gray-800 shadow-sm">Simpan Pengeluaran</button>
            </div>
          </form>
        </div>
      )}
      ...
      <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden mt-8">
        <div className="p-6 border-b border-gray-100 flex flex-col md:flex-row justify-between md:items-center gap-4">
          <h2 className="text-lg font-semibold text-gray-900">Riwayat Pengeluaran</h2>
          <div className="flex gap-3 w-full md:w-auto">
            <div className="relative flex-1 md:w-64">
              <Search className="w-4 h-4 absolute left-3 top-1/2 -translate-y-1/2 text-gray-400" />
              <input type="text" placeholder="Cari referensi tagihan..." className="w-full pl-9 pr-4 py-2 bg-gray-50 border border-gray-200 rounded-lg text-sm focus:ring-2 focus:ring-gray-900 outline-none" />
            </div>
           ...
           <table className="w-full text-left text-sm text-gray-500">
            ...
            <tbody className="divide-y divide-gray-50">
              {expenseList.length === 0 ? (
                <tr><td colSpan={5} className="px-6 py-12 text-center text-gray-500">Belum ada data pengeluaran. Silakan tambah data baru.</td></tr>
              ) : expenseList.map((trx) => (
                <tr key={trx.id} className="hover:bg-gray-50/50">
                  ...
                  <td className="px-6 py-4 text-right font-semibold text-red-600">
                    <div className="flex items-center justify-end gap-1.5"><ArrowDownRight className="w-3.5 h-3.5" /> Rp {trx.amount.toLocaleString("id-ID")}</div>
                  </td>
